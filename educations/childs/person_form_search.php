...
$pid = trim($_GET["pid"]);
$nme 	= trim($_GET["name"]);
$surnme = trim($_GET["sname"]);
$perid = $_GET["perid"];

if($pid){	
	$sql = "SELECT a.perid,a.pid,CONCAT(titnme,`name`,' ',`sname`) AS nme,genid,a.adr,soi,a.vllid,e.plcnmegen AS plc,a.pertel
    FROM person a 
    LEFT JOIN `const_plcnmegen` e ON a.plcid=e.plcidgen
    LEFT JOIN titname pre ON a.titid=pre.titid ";
...
if(($nme)|($surnme)){	
    ...
    LEFT JOIN `const_plcnmegen` e ON a.plcid=e.plcidgen
    LEFT JOIN titname pre ON a.titid=pre.titid ";
	$sql.= " where name like '%$nme%' and sname like '%$surnme%' and a.perid<>'$perid'";
    $result = mysqli_query($conn,$sql);
    $numRows = @mysqli_num_rows($result);
    // echo $sql;
}
                    ...
                    <table class="table table-bordered">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th scope="col">ที่อยู่ปัจจุบัน</th>
                                <th scope="col">เลือก</th>
                            </tr>
                        </thead>
                        <!-- เริ่มวนรอบแสดงข้อมูล -->
                    <?php //SELECT a.perid,a.pid,CONCAT(titnme,`name`,' ',`sname`) AS nme,genid,a.adr,a.vllid,e.plcnmegen AS plc,a.pertel
                        $n = 0;
...
function sendValue(e1) {
    ...
    window.open("../?page=childs&function=add&id=" + e1);
