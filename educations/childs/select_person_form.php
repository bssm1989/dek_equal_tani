...
if($pid){
	// $sql = "select perid,pid,concat(prenme,' ',nme,' ',surnme) as nme,adr,vllnogen,plcnmegen";
	// $sql.= " from person p left join const_prenme pre on p.preid=pre.preid";
	// $sql.= " left join const_plcnmegen plc on p.plcidgen=plc.plcidgen";
	// $sql.= " where stu_pid='$stu_pid'"; #and pid<>'' and perid<>'$perid'";
	
    /* $sql = "SELECT a.stu_id,stu_pid,CONCAT(prenme,stu_name) AS nme,stu_surname,stu_roladr,stu_rolvllno,e.plcnmegen AS rolplc";
	$sql.= " from student a left join c_plcnmegen e on a.stu_rolplcid=e.plcidgen";
    $sql.= " left join constpre pre on a.stu_preid=pre.preid ";
    $sql.= " where stu_pid='$stu_pid' and stu_pid<>'' and stu_id<>'$stu_id'"; */
	//echo "xxxxx".$sql;
	
	$sql = "SELECT a.perid,a.pid,CONCAT(prenme,nme,' ',surnme) AS nme,a.sex,a.adr,a.soi,a.vllid,e.plcnmegen AS rolplc,a.tel
    FROM person a LEFT JOIN const_plcnmegen e ON a.rolplcid=e.plcidgen
    LEFT JOIN constpre pre ON a.preid=pre.preid ";
...
if(($nme)|($surnme)){	
	$sql = "SELECT a.perid,a.pid,CONCAT(prenme,nme,' ',surnme) AS nme,a.sex,edulevnme,occnme,a.sal,a.roladr,a.rolvllno,e.plcnmegen AS rolplc,a.wrkplace,a.wrkvllno,w.plcnmegen AS wrkplc,a.tel
    FROM person a LEFT JOIN const_plcnmegen e ON a.rolplcid=e.plcidgen
    LEFT JOIN c_plcnmegen w ON a.wrkplcid=w.plcidgen
    LEFT JOIN constedulev c ON a.edulev=c.edulev
    LEFT JOIN constocc o ON a.occid=o.occid
    LEFT JOIN constpre pre ON a.preid=pre.preid ";
	$sql.= " where nme like '%$nme%' and surnme like '%$surnme%' and perid<>'$perid'";
    $result = mysqli_query($conn,$sql);
    $numRows = @mysqli_num_rows($result);
}
...
function sendValue(e1) {
    ...
    window.open("../?page=persons&function=update&id=" + e1);
