...
function insertData($table, $data, $conn)
{
    ...
    try {
        ...
        if (isset($action) && $action === 'insert') {
            ...
            $sql = "UPDATE $table SET $columns = $values WHERE perid = :perid";
            // set new array  remove perid
             $new_data = array_filter($data, function ($value) {
                $value !== 'perid';
            });

           
            $valuesString = '"' . implode('", "', array_map('addslashes', array_values($new_data))) . '"';
