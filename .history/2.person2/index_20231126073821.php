                            ...
                            <tr id="row_<?= $perid ?>">
                                ...
                                <td>
                                    ...
                                    <div class="btn-group" role="group">
                                        <a href="?page=<?= $_GET['page'] ?>&function=add&perid2=<?= $perid ?>" class="btn btn-warning text-white"><i class="fas fa-edit"></i></a>
...
<script>
    function deletePerson(id, name, lastName) {
        Swal.fire({
            title: "ลบข้อมูล",
            html: `<p>คุณต้องการลบข้อมูลของ ${name} ${lastName} ใช่หรือไม่?</p>
         <input type="text" id="confirmName" class="swal2-input" placeholder="พิมพ์ชื่อของคุณที่นี่">`,
            icon: "warning",
            ...
            preConfirm: () => {
                const confirmName = Swal.getPopup().querySelector("#confirmName").value;
                if (confirmName === `${name}`) {
                    return true; // Proceed with the delete action
                } else {
                    Swal.showValidationMessage("ชื่อไม่ถูกต้อง"); // Show an error message
                    return false;
                }
            }
        }).then((result) => {
            if (result.isConfirmed) {
                ...
                deletePersonData2(id);
            }
        });
    }

    function deletePersonData2(id) {
