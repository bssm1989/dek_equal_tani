...
if ($_GET["tab"] == "1") {
    ...
    $sql = "SELECT h.perid, CONCAT(p.name, ' ', p.sname) AS person_fullname,
            ...
            CONCAT(s.name, ' ', s.sname) AS recorded_by_name,
            CONCAT(m.name, ' ', m.sname) AS modified_by_name
            FROM hedu AS h
...
<?php if ($tab == 1 || $_GET["perid"]) { ?>
    <ul class="nav nav-tabs">
        <li class="nav-item">
            <a class="nav-link " href="?page=2.person2&function=add&perid2=<?= $peridFromHedu ? $peridFromHedu : $perid2 ?>">ประวัติบุคคล</a>
        </li>
        <li class="nav-item">
        ...
        <li class="nav-item">
            ...
            <a class="nav-link" href="?page=5.helpeducation&function=add&perid2=<?= $heduid ?>&tab=1">5.helpeducation</a>
        </li>
        <li class="nav-item">
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                Swal.fire({
                                    ...
                                    // Go to next page
                                    window.location.href = "?page=3.historyeducation&function=add&perid2=" + data['perid'] + "&tab=1";
                                });
                            } else {
                                // Show error message
                                Swal.fire({
