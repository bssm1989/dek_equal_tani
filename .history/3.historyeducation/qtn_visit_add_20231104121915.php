...
if ($_GET["tab"] == "1") {
    ...
    $perid2 = $_GET["perid2"];
    if (isset($_GET["heduid"]) && $_GET["heduid"] != "") {
        $heduid2 = $_GET["heduid"];
...
<?php if ($tab == 1 || $_GET["perid"]) { ?>
    <ul class="nav nav-tabs">
        <li class="nav-item">
            <a class="nav-link " href="?page=2.person2&function=add&perid=<?= $heduid ? $heduid : $perid2 ?>">ประวัติบุคคล</a>
        </li>
        <li class="nav-item">
...
<div class="row justify-content-between card-header text-right mb-0">
    ...
    <div class="col-auto">
        <button class="btn app-btn-secondary" type="button" onclick="window.history.back();">ย้อนกลับ2</button>
    </div>
</div>
