<?php
$hhtrnid = $_GET["hhtrnid"] ?? "";
$perid2 = $_GET["perid2"] ?? "";
$tab = $_GET["tab"] ?? "";

// Determine the column for the SQL query based on the existence and value of hhtrnid
$column = isset($_GET["hhtrnid"]) && $_GET["hhtrnid"] != "" ? "ht.hhtrnid" : "ht.perid";
...
if ($_GET["tab"] == "1") {
    ...
    $sql = "SELECT ht.hhtrnid, ht.perid, ht.htrndtestr, ht.htrndteend, ht.htrntit, ht.prvid, ht.htrndetail,
            ...
            WHERE $column = " . ($_GET["hhtrnid"] ?? $_GET["perid2"]);
} else if ($hhtrnid) {
    // Construct your SQL query based on hhtrnid
    $sql = "SELECT ht.hhtrnid, ht.perid, ht.htrndtestr, ht.htrndteend, ht.htrntit, ht.prvid, ht.htrndetail,
            CONCAT(p.name, ' ', p.sname) AS participant_name, prv.prvnme AS training_province,
            ht.recorded_by, ht.recorded_date, ht.modified_by, ht.modified_date,
            ht.recorded_by, ht.recorded_date, ht.modified_by, ht.modified_date 
...
<ul class="nav nav-tabs">
  <li class="nav-item">
    <a class="nav-link "  href="?page=2.person2&function=add&perid2=<?= $hflwid ?>tab=1">2.person</a>
  </li>
    <li class="nav-item">
    ...
    <li class="nav-item">
        ...
        <a class="nav-link" href="?page=4.works&function=add&perid2=<?= $hflwid ?>tab=1">4.works</a>
    </li>
    <li class="nav-item">
        <!-- ?page=5.helpeducation&function=add&perid2=3 -->
        <a class="nav-link" href="?page=5.helpeducation&function=add&perid2=<?= $hflwid ?>&tab=1">5.helpeducation</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?page=6.1institute&function=add&perid2=<?= $hflwid ?>tab=1">6.1institute</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?page=7.history_help_job&function=add&perid2=<?= $hflwid ?>tab=1">7.history_help_job</a>
    </li>
    <li class="nav-item">
        <a class="nav-link" href="?page=8.htraining&function=add&perid2=<?= $hflwid ?>tab=1">8.htraining</a>
    </li>
    <li class="nav-item">
        <a class="nav-link active" aria-current="page" href="#">9.hfolowup</a>
    </li>
...
<div class="mb-0 text-right row justify-content-between card-header">
    <div class="col-auto">
        <?php if (isset($person_fullname)) { ?>
            <h4 class="mb-0 app-page-title"> จัดการข้อมูลประวัติการติดตาม/การเยี่ยมเยียน ของ<?php echo $person_fullname; ?></h4>
        <?php } else { ?>
            <h4 class="mb-0 app-page-title"> จัดการข้อมูลประวัติการติดตาม/การเยี่ยมเยียน</h4>
        <?php } ?>
    </div>
    <div class="col-auto">
...
• รายละเอียดการติดตาม/เยี่ยมเยียน -->
    ...
    <div class="mb-3 col-12 col-sm-4">
        ...
        <!-- //div group -->
        <!-- input hidden hflwid -->
        <input type="hidden" class="form-control" name="hflwid" id="hflwid" placeholder="" value="<?= $hflwid; ?>" readonly="true">
        <div class="input-group">
            <!-- Search for a person... to Thai -->
            <input type="text" id="personSelect" name="personName" class="form-control" placeholder="ค้นหาบุคคล" value="<?= $participant_name; ?>">
