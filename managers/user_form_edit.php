<?php
    ...
    if($ofcid){
        $sql = "SELECT a.`ofcid`,a.`ofcnme`,a.usrnme,a.pwd,a.`pio`,a.`status`,a.`profile_photo`,a.updated_at,b.`optid` AS optid,b.`optnme`,`email`
        FROM `ofc` a INNER JOIN (SELECT `optid`,`optnme` FROM `opt`) b ON a.`optid` = b.`optid` ";
        $sql.= " WHERE a.ofcid=$ofcid ";
...
<div class="row justify-content-between card-header text-right mb-0">
    <div class="col-auto">
        <h4 class="app-page-title mb-0"> แก้ไขข้อมูลผู้ดูและระบบ</h4>
    </div>
    <div class="col-auto">
...
<hr class="mb-0">
<div class="row gy-4">
    <form name="frmUser" id="frmUser" method="post" action="./managers/perupfile.php" enctype="multipart/form-data" onSubmit="" target="">
        <div class="row">
                ...
                <div class="app-card app-card-account shadow-sm d-flex flex-column align-items-start">
                    ...
                    <div class="app-card-body px-4 w-100">
                                    ...
                                    <div class="item-data">
                                        ...
                                        <span style="position: relative; top: 5px;">
                                            ...
                                            <div class="input-group mb-3">
                                                ...
                                                <?php if($file_resnme) echo "<a href=\"./managers/perdelfile.php?ofcid=".$ofcid."&filename=".$picname."\" type='button' class='btn app-btn-secondary' >ลบ</a>"; ?>
                                    ...
                                    <div class="item-data">
                                        <div class="form-check form-check-inline"><br>
                                            <input id="permission1" name="permission" type="radio" value="1"
                                                ...
                                                <?php if($rows["pio"]=="1") echo "checked";?> required>
                                            <label class="custom-control-label" for="permission1">1.ผู้ดูแล</label>
                                        </div>
                                        <div class="form-check form-check-inline"><br>
                                            <input id="permission2" name="permission" type="radio" value="2"
                                                class="custom-control-input"
                                                <?php if($rows["pio"]=="2") echo "checked";?> required>
                                        ...
                                        <div class="form-check form-check-inline"><br>
                                            <input id="permission3" name="permission" type="radio" value="3"
                                                class="custom-control-input"
                                                <?php if($rows["pio"]=="3") echo "checked";?> checked required>
                        ...
                        <div class="item border-bottom py-3">
                            <div class="row justify-content-between align-items-center">
                                <div class="col-auto">
                                    <div class="item-label"><strong>สถานะการใช้งาน</strong></div>
                                    <div class="item-data">
                                        <div class="form-check form-check-inline"><br>
                                            <input id="ofc_status0" name="ofc_status" type="radio" value="0"
                                                class="custom-control-input"
                                                <?php if($rows["status"]=="0") echo "checked";?> checked required>
                                            <label class="custom-control-label" for="ofc_status0">0.รออนุมัติ</label>
                                        </div>
                                        <div class="form-check form-check-inline"><br>
                                            <input id="ofc_status1" name="ofc_status" type="radio" value="1"
                                                class="custom-control-input"
                                                <?php if($rows["status"]=="1") echo "checked";?> required>
                                            <label class="custom-control-label" for="ofc_status1">1.เปิดใช้งาน</label>
                                        </div>
                                        <div class="form-check form-check-inline"><br>
                                            <input id="ofc_status2" name="ofc_status" type="radio" value="2"
                                                class="custom-control-input"
                                                <?php if($rows["status"]=="2") echo "checked";?> required>
                                            <label class="custom-control-label" for="ofc_status2">2.ปิดการใช้งาน</label>
                                        </div>
                                    </div>
                                </div>
                                <!--//col-->
                    ...
                    <a class="btn app-btn-secondary" href="#"
                            onClick="if(checkFullUser('กรุณากรอกชื่อเต็มของผู้ใช้งานก่อนค่ะ/ครับ')==true){if(confirm('ต้องการบันทึกข้อมูลหรือไม่')==true) saveUserUpdate()};">อัพเดทข้อมูล</a>
                    </div>
                    <!--//app-card-footer-->
                    ...
                    <div class="app-card-body px-4 w-100">
                                ...
                                <div class="col-md-9">
                                    ...
                                    <div class="item-data">
                                        <input type="text" class="form-control" id="usrnme" name="usrnme"
                                            value="<?=$rows["usrnme"];?>" placeholder="usrnme" required>
                                    </div>
                                </div>
                                <!--//col-->
                                <div class="col text-end">
                                <a class="btn-sm app-btn-secondary" href="#"
                                        onClick="if(checkUser('กรุณากรอกชื่อผู้ใช้งานก่อนค่ะ/ครับ')==true){if(confirm('ต้องการเปลี่ยนชื่อผู้ใช้งานหรือไม่')==true) changeUser()};">เปลี่ยนชื่อผู้ใช้งาน</a>
                                </div>
                                <!--//col-->
                            </div>
                            <!--//row-->
                        ...
                        <div class="item mb-3">
                            <div class="row justify-content-between align-items-center">
                                ...
                                <div class="col text-end">
                                <a class="btn-sm app-btn-secondary" href=""
                                        onClick="if(checkPwd('กรุณากรอกรหัสผ่านก่อนค่ะ/ครับ')==true){ if(confirm('ต้องการเปลี่ยนรหัสผ่านหรือไม่')==true) changeUserPWD()};">เปลี่ยนรหัสผ่าน</a>
                                </div>
                                <!--//col-->
                            </div>
                            <!--//row-->
...
function checkFullUser(e1) {
    if (document.frmUser.ofcnme.value.length == 0) {
        alert(e1);
        return false;
    }
    return true;
...
function checkUser(e1) {
    if (document.frmUser.usrnme.value.length == 0) {
        alert(e1);
        return false;
    }
    return true;
...
function checkPwd(e1) {
    if (document.frmUser.password.value.length == 0) {
        alert(e1);
        return false;
    }
    return true;
